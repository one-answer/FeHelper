...
new Vue({
    ...
    methods: {
        compress: function() {
            ...
            this.sourceContent ? "js" === this.codeType ? this.jsCompress(this.sourceContent) : "css" === this.codeType ? this.cssCompress(this.sourceContent) : this.htmlCompress(this.sourceContent) : alert("请先粘贴您需要压缩的代码")
        },
        changeCodeType(e) {
            ...
            editor.getValue().trim() && this.$nextTick(this.compress)
        },
        buildCompressInfo(e, t) {
            let s = e => String(e).split("").reverse().join("").replace(/(...)(?!$)/g, "$1,").split("").reverse().join("")
              , o = e.length - t.length
              , r = e.length ? (100 * o / t.length).toFixed(2) : 0;
            this.compressInfo = "压缩前: <strong>" + s(e.length) + "</strong>，压缩后: <strong>" + s(t.length) + "</strong>，压缩率: <strong>" + s(o) + " (" + r + "%)</strong>"
        },
        jsCompress(e) {
            let t = UglifyJs3.compress(e);
            this.hasError = !!t.error,
            this.resultContent = t.out || t.error,
            !this.hasError && this.buildCompressInfo(this.sourceContent, this.resultContent)
        },
        cssCompress(e) {
            let t = e.replace(/\/\*(.|\n)*?\*\//g, "").replace(/\s*([\{\}\:\;\,])\s*/g, "$1").replace(/\,[\s\.\#\d]*\{/g, "{").replace(/;\s*;/g, ";").match(/^\s*(\S+(\s+\S+)*)\s*$/);
            this.resultContent = null === t ? e : t[1],
        ...
        htmlCompress(e) {
            ...
            t.log = console.log;
            try {
                this.resultContent = require("html-minifier").minify(e, t),
                this.buildCompressInfo(this.sourceContent, this.resultContent)
            } catch (e) {
                this.hasError = !0,
        ...
        toast(e) {
            ...
            if (t)
                ...
                s.innerHTML = '<div id="fehelper_alertmsg">' + e + "</div>",
                t = s.childNodes[0],
